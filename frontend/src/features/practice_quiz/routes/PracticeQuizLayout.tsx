...
export default function PracticeQuizLayout() {
  ...
  const [step, setStep] = useState<"welcome" | "setup" | "quiz" | "results">(
    "setup",
  );
  ...
  const handleStartQuiz = async (
    ...
    const selectedQuestions = await practiceQuiz(
      topics.join(","),
      questions,
      level,
    );
  ...
  const onBack = () => {
    // localStorage.removeItem("quizState");
    navigate(
      `/roadmap?selectedSkill=${topic}&score=${topicScore}&preferredLanguage=${preferredLanguage}`,
    );
  };
  if (isLoading) {
      ...
      <main className="flex-1 flex flex-col">
        ...
        {step === "results" && (
          <PracticeQuizResults
            ...
            onReset={handleReset}
            onTryAgain={() =>
              handleStartQuiz(numQuestions, difficulty, selectedTopics)
            }
          />
        )}
      </main>
